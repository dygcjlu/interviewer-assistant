...
| 3 | 当前候选人信息：姓名、职位、工作年限、技能、简历内容（前 1500 字）、面试简报预览（前 800 字）、可选历史面试摘要（前 1200 字） | 选中候选人 / brief_done 时注入，切换时替换 |
...
| `set_candidate_context(profile, interview_brief?, history_summary?)` | 切换候选人或 brief_done 时调用，替换系统提示第 3 层 |
...
| 方法 | 说明 |
|---|---|
| `create_session(candidate_id?)` | 创建 InterviewSession，从文件加载候选人历史记忆、简历与简报 |
| `start_interview()` | 激活 InterviewAgent，注册压缩回调，启动音频（含 `on_round_finalized`），`stage=interviewing`，调用 `memory.start_interview(session)` |
| `stop_interview()` | flush pending round，停止音频，设置 evaluating/completed |
| `close_session()` | 调用 `memory.finish_interview(session)`（写 transcript / 归档 WAL / 更新 index），`context_manager.reset()`，清空会话 |
...
- `parse_done`：按真实姓名判重；命中则写入 `pending_duplicates` 并返回 `duplicate_candidate`；否则 `save_candidate` → 更新 `resume_content` → 刷新 MainAgent Layer 3
- `brief_done`：`save_brief` 落盘 → 异步 `_generate_questions_from_brief`（写 `questions.json`）→ 更新 `session.interview_brief` → `set_candidate_context(..., history_summary?)` 刷新第 3 层（**不**调用 `start_interview`）
...
ResumeAgent 读取 candidates/{cid}/profile.md，生成结构化简报
    ↓
返回 brief_done → dispatch_to_agent 触发副作用（save_brief + 异步生成问题清单 + 刷新 Layer 3）
    ↓
MainAgent 流式回复用户；用户点击「开始面试」后由 InterviewController 写 session.json
```
...
**追问输出格式**：`generate_suggestion()` 使用流式 `chat_stream()`，要求 LLM 直接输出**一句中文追问话术**，不使用 JSON action 格式。内容通过 `suggestion_delta` / `suggestion_final` 推送至前端。
...
| `generate_suggestion(request_id)` | SuggestionTrigger 回调 / 手动触发 | `chat_stream` 流式 yield；超过 token 预算时截断历史或跳过 |
| `cancel_current_stream()` | 切换到 manual 模式 / 新触发时 | 取消正在进行的 LLM 生成 task |
