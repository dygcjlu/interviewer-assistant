...
| InterviewAgent | `file_read` |
| EvalAgent | `skill_view`（不使用 PromptBuilder，仅在 BaseAgent._run_with_tools 中使用） |
...
- 每道题下展示 `follow_ups` 预设追问点（格式：`追问: {内容}`），供 LLM 判断当前维度是否已充分覆盖
- `已覆盖维度` 来自 `session.covered_dimensions`
- **简历内容**：`resume_content` 非空时直接展示完整 Markdown 正文；为空时展示文件路径（供 `file_read` 工具读取）
- **岗位要求**：`user_memory` 由 PromptBuilder 从 `UserMemoryStore` 缓存，非空时追加在本层末尾

**更新时机**：每次 `build()` 调用时实时读取 `session` 对象，反映最新题目覆盖状态；USER.md 变更后需调用 `prompt_builder.reload_user_memory()` 刷新
...
- 题目 JSON: resumes/张三_questions.json
```

ResumeAgent 执行后返回：
- `{"type": "parse_done", "profile": {...}, "markdown_path": "resumes/张三.md"}`
- `{"type": "questions_done", "questions": [...]}`
...
# 再追加本轮的最新对话内容（含连续追问计数和未覆盖题目）
user: 面试官：{interviewer_text}
      候选人最新回答：{candidate_text}

      当前话题已连续追问次数：{_consecutive_follow_ups}
      未覆盖题目（共 N 题）：
      - [维度] 题目...
...
LLM 返回严格 JSON：`{"action": "follow_up" | "switch_topic" | "skip", "text": "..."}`

- `follow_up`：`_consecutive_follow_ups += 1`，向前端推送 suggestion text
- `switch_topic`：`_consecutive_follow_ups = 0`，向前端推送新话题问题
- `skip`：不推送任何内容（`suggestion_final` 携带 `skipped: true`，前端静默处理）

注意 `self._history` 是 InterviewAgent 自己维护的追问对话历史（最近 10 轮，20 条消息），独立于 ContextManager 的面试对话轮次。历史中存储完整 JSON 响应，供 LLM 参考过往决策。
...
```
[0] system  — Layer 1~6 合并（\n\n 拼接）：
              INTERVIEW_AGENT_SYSTEM_PROMPT（含三种 action 说明和 JSON 输出格式）
              ＋ "可用面试技巧：deep_dive: ..."（Layer 2）
              ＋ "可用工具：file_read: ..."（Layer 3，若有）
              ＋ "候选人 张三 历史面试记录：..."（Layer 4，若有）
              ＋ "候选人：张三\n当前职位：...\n## 候选人简历\n{resume_content}\n面试题目清单：\n○ [系统设计] ...\n追问: ...\n..."（Layer 5）
              ＋ "[以下为早期面试对话的压缩摘要...] ..."（Layer 6，若有）

[1] user    — "面试官：你之前做过哪些高并发项目？\n候选人：我在上家公司负责了一个日活500万的电商系统..."
              （Layer 7 窗口第 1 轮，面试官+候选人合并）
[2] assistant — "[追问建议] 这个系统的峰值 QPS 大概是多少？..."
              （第 1 轮的 llm_suggestion，若有）

... （Layer 7 最多 6 轮，每轮 1-2 条）

[N]   user    — "面试官：那你们数据库层怎么做的？\n候选人：...\n\n当前话题已连续追问次数：1\n未覆盖题目（共 3 题）：..."
               （InterviewAgent._history：上一轮追问的 user 消息）
[N+1] assistant — "{\"action\": \"follow_up\", \"text\": \"你们使用了分库分表方案...\"}"
               （InterviewAgent._history：上一轮追问的 assistant 消息，完整 JSON）

[N+2] user  — "面试官：请讲一下你们的缓存策略\n候选人最新回答：我们用 Redis 做了...\n\n当前话题已连续追问次数：2\n未覆盖题目（共 3 题）：...\n\n请判断下一步行动并输出 JSON。"
              （本轮触发内容，由 generate_suggestion() 追加）
```

> **注**：Layer 5 固定区现在直接包含 `resume_content`（简历 Markdown 正文）而非仅文件路径，Agent 无需再调用 `file_read` 工具即可访问完整简历。
> 若 `resume_content` 为空（如创建会话时未加载），则退化为显示 `candidates/{id}/profile.md` 路径提示。
