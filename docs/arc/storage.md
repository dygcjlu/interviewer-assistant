...
> **存储后端**：基于文件系统的纯文本存储。所有候选人档案、面试记录、评价报告均以 Markdown + YAML frontmatter（及 JSON）形式存放在 `candidates/` 目录下。
...
└── {candidate_id}/                   # 每个候选人一个 UUID 子目录
    ...
    ├── brief.md                      # 面试简报（Markdown，由 ResumeAgent 生成）
    ├── questions.json                # 结构化问题清单（简报后异步生成）
    ├── resume.pdf                    # 原始 PDF（上传时复制至此）
    └── interviews/
        ├── index.md                  # 本候选人的面试历史摘要
        └── {interview_id}/           # 每次面试一个 UUID 子目录
            ├── session.json          # 会话元数据（阶段、时间、录音路径）
            ├── rounds.jsonl          # 进行中 WAL；finish 后归档为 rounds.jsonl.archived
...
| `strengths` | 优势列表 |
| `weaknesses` | 不足列表 |
| `dimensions` | 维度评分列表（`dimension` / `score` / `comment` / `evidence`） |

---

### `candidates/{id}/questions.json`

结构化面试问题清单（JSON 数组），由 `brief_done` 后异步 `_generate_questions_from_brief` 写入。

典型字段：`id`、`question`、`focus`、`covered`、`covered_by`（`manual` / `auto` / 空）。

---

## MemoryModule 职责划分

**文件**：`src/storage/memory_module.py`（Facade）、`candidate_store.py`、`interview_store.py`、`eval_store.py`、`_store_common.py`

`MemoryModule` 是唯一对外暴露的存储接口（Facade 模式），将候选人、面试、评价三类操作分别委托给 `CandidateStore`、`InterviewStore`、`EvalStore`。共享的 frontmatter 解析、index 构建等纯函数放在 `_store_common.py`。所有 public 方法签名与拆分前保持一致，历史 `from ...memory_module import InterviewDetail` 等 import 路径不变。

各 Store 的读写操作均通过原子写入（`mkstemp + os.replace`）保证文件安全，避免写入中途崩溃损坏数据。`rebuild_index()` 留在 Facade 层编排三个 Store。
...
| 方法 | 触发时机 | 说明 |
|---|---|---|
| `start_interview(session)` | `InterviewController.start_interview()` | 写 `session.json`（stage=interviewing） |
| `append_round(candidate_id, interview_id, round)` | 每轮 `on_round_finalized` | 追加 `rounds.jsonl` WAL |
| `finish_interview(session)` | `close_session()` 时 | 写 `transcript.md`，更新 `session.json`，归档 WAL → `.archived`，更新两级 index |
| `scan_orphan_wal()` / `recover_interview_from_wal()` / `discard_orphan_wal()` | Recovery API / 启动扫描 | 崩溃残留 WAL 处理 |
...
| 方法 | 说明 |
|---|---|
| `save_questions(candidate_id, questions)` | 写 `questions.json` |
| `get_questions(candidate_id)` | 读问题清单，不存在返回 `[]` |
| `update_question_coverage(candidate_id, question_id, covered, covered_by)` | 更新单题覆盖状态 |
