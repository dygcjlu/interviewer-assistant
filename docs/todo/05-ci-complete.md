...
- [x] push 到 main 分支时 CI 自动触发（workflow 配置已确认自洽）
  - 实现：`.github/workflows/ci.yml` 第 3–7 行 `on: push: branches: [main]`（同时对所有分支的 PR 触发）。
- [x] CI 全部通过（本地验证：`ruff check .` + `pytest tests/unit tests/integration --cov-fail-under=60` 通过，548 项测试、覆盖率 62.44%；合并到 main 后需在 GitHub Actions 确认远端 CI 绿灯）
