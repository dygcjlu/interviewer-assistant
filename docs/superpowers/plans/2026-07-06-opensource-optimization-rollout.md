...
- [x] **Step 2: 本地确认当前覆盖率 ≥ 60%**（实际 58.48%，未达标，按 report 记录留待阶段三/收尾补齐）
...
- [x] **Step 2: 改造测试隔离环境变量**（实际变量名为 `VOLC_APP_KEY`/`VOLC_ACCESS_KEY`，非简报示例的 `VOLC_APP_ID`/`VOLC_ACCESS_TOKEN`，已按实际代码修正，审查已确认）
...
- [x] **Step 3: 复跑测试确认通过（并在有凭证的环境下再跑一次）**
...
- [x] **Step 1: 写失败测试（精确计数 > 空内容基线，且分区求和自洽）**（修复轮后改为 `MagicMock` + `assert_called_once()` 断言，真正验证"单次调用"约束）
...
- [x] **Step 3: 引入虚拟消息列表构造 + 改写 `_estimate_tokens`**
...
- [x] **Step 4: 改写 `token_usage` 属性分区口径**（三次独立调用与全局"单次调用"约束的文字表述存在张力；经审查数学验证无实际计数虚高，已在 design doc 补充"已记录的例外"说明）
...
- [x] **Step 5: 改写 `_compress_async` 里的 tail 边界与压缩可行性估算**（tail 边界改为对逐步扩大的候选列表整体重算一次 `count_tokens`，而非按 brief 示例逐轮累加求和，更贴合约束精神）
...
```
（提交 e15c882；修复轮追加提交 5a5981a：强化单测断言 + 补充 design doc 例外说明，批次审查 Approved）
...
- [x] **Step 1: 改写估算逻辑**（实际属性名为 `self.llm_client`，已按代码核实调整）
...
- [x] **Step 2: 补/跑测试验证阈值分流仍正确**（用 `MagicMock` + `assert_called_once()` + 消息列表长度校验，正确验证"单次调用"约束）
...
```
（提交 90704eb；批次审查 Approved，无需修复轮）
...
### Task 7.2: 新建 `candidate_store.py` 并迁移候选人相关方法（已完成，commit `6ba12ff`，批次审查 Approved，见 Task 7.5 汇总）
...
### Task 7.3: 新建 `interview_store.py` 并迁移面试生命周期 + WAL 方法（已完成，commit `6c1a65c`，批次审查 Approved，见 Task 7.5 汇总）
...
### Task 7.4: 新建 `eval_store.py` 并迁移评价报告持久化（已完成，commit `71fb285`，批次审查 Approved，见 Task 7.5 汇总）
...
### Task 7.5: 确认 `MemoryModule` Facade 接口零变化（已完成，Step D 零遗漏，独立审查 Approved）

**Files:**
- Modify（如需补委托）: `src/storage/memory_module.py`

- [x] **Step 1: 逐一核对 public 方法仍存在且签名不变**

用 Grep 列出改造前 `MemoryModule` 所有 `async def`/`def`（非 `_` 前缀）方法名，逐一确认改造后仍在 Facade 上且参数/返回一致。差异清零。

实际结果：实现者用 AST 解析新旧文件比对全部 25 个 public 方法，0 处不一致；reviewer 额外独立抽查 12/25 个方法签名确认无误。

- [x] **Step 2: `ruff check` + import 兼容检查**
...
实际结果：`ruff check src\storage\` → All checks passed；import 冒烟测试打印 `ok`；reviewer 独立重跑确认一致。

- [x] **Step 3: Commit（如有补充）**

Step D 验证未发现任何遗漏或行为差异，working tree clean，未产生第 4 次 commit。

**Tasks 7.2–7.5 批次审查结论（Approved）：** 零 Critical/Important。2 项 Minor 不阻塞：① `rebuild_index` 直接访问 `CandidateStore._write_candidates_index`（私有方法），与 `InterviewStore` 侧已提供 public 包装的做法不完全对称，brief 已明确允许，留待后续加 `write_candidates_index` public 包装即可收敛；② `memory_module.py` 顶部为向后兼容 re-export 的 `_store_common` 导入缺少说明性注释。`InterviewStore`/`EvalStore` 交叉依赖问题采用方案 B（`InterviewStore.get_interview_detail_without_eval()` + Facade 用 `dataclasses.replace` 组装 `eval_report`），避免了方案 A（二次注入）引入的循环依赖与初始化时序陷阱。reviewer 独立重跑全量测试 530 passed + ruff clean，并抽查 WAL 三方法（`scan_orphan_wal`/`recover_interview_from_wal`/`discard_orphan_wal`）、`get_candidate_history` 归属、`save_eval_report` 孤儿兜底路径、全部历史 import 兼容点，均确认零回归。
