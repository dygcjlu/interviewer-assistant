...
_BAILIAN_THINK = ThinkingConfig(extra_body={"enable_thinking": True}, suppress_temperature=False, reasoning_effort=None)
...
class TestBuildRequestKwargs:
    ...
    def test_deepseek_official_suppresses_temperature(self) -> None:
        ...
        kwargs = build_request_kwargs(cfg, _DS_OFFICIAL_THINK, temperature=0.1, timeout=30.0)
...
class TestFilterConfigs:
    ...
    def test_filter_by_keyword_case_insensitive(self) -> None:
        filtered_lower = filter_configs(MODEL_CONFIGS, filter_str="qwen", skip_thinking=False)
        filtered_upper = filter_configs(MODEL_CONFIGS, filter_str="QWEN", skip_thinking=False)
    ...
    def test_nonexistent_filter_returns_empty(self) -> None:
        filtered = filter_configs(MODEL_CONFIGS, filter_str="nonexistent_xyz", skip_thinking=False)
        assert filtered == []
    ...
    def test_think_variants_have_with_think_set(self) -> None:
        for cfg in MODEL_CONFIGS:
            if "+think" in cfg.label:
                assert cfg.with_think is not None, f"{cfg.label}: with_think should not be None"

    def test_non_think_variants_no_extra_body(self) -> None:
        for cfg in MODEL_CONFIGS:
            if "+think" not in cfg.label:
                kwargs = build_request_kwargs(cfg, cfg.no_think, temperature=0.1, timeout=30.0)
                assert "extra_body" not in kwargs, f"{cfg.label}: should not have extra_body in no_think mode"

    def test_all_labels_unique(self) -> None:
        labels = [c.label for c in MODEL_CONFIGS]
        assert len(labels) == len(set(labels)), "Duplicate labels found in MODEL_CONFIGS"

    def test_dashscope_configs_use_llm_api_key(self) -> None:
        for cfg in MODEL_CONFIGS:
            if "dashscope" in cfg.base_url:
                assert cfg.api_key_env == "LLM_API_KEY", f"{cfg.label}: should use LLM_API_KEY"

    def test_deepseek_official_configs_use_deepseek_api_key(self) -> None:
        for cfg in MODEL_CONFIGS:
            if cfg.base_url == "https://api.deepseek.com":
                assert cfg.api_key_env == "DEEPSEEK_API_KEY", f"{cfg.label}: should use DEEPSEEK_API_KEY"
