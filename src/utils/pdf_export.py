...
def build_report_pdf(report: EvalReport, candidate_name: str = "") -> bytes:
    ...
    h2 = ParagraphStyle("h2", fontName=font, fontSize=13, leading=18, spaceBefore=10, spaceAfter=4, textColor=colors.HexColor("#1a56db"))
    body = ParagraphStyle("body", fontName=font, fontSize=10, leading=15)
    small = ParagraphStyle("small", fontName=font, fontSize=9, leading=13, textColor=colors.grey)
    label = ParagraphStyle("label", fontName=font, fontSize=10, leading=14, fontWeight="bold")

    rec = _RECOMMENDATION_LABELS.get(report.recommendation, report.recommendation)
    rec_color = colors.HexColor("#15803d") if "hire" in report.recommendation else colors.HexColor("#b91c1c")
    ...
    rec_style = ParagraphStyle("rec", fontName=font, fontSize=13, leading=18, textColor=rec_color)
