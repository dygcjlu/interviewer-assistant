...
class MemoryModule:
    ...
    def __init__(self, candidates_dir: str = "candidates") -> None:
        ...
        self._evals = EvalStore(self._root, self._candidates, self._interviews)
    ...
    async def get_latest_eval_report(self, candidate_id: str) -> EvalReport | None:
        return await self._evals.get_latest_eval_report(candidate_id)
    ...
    async def get_interview_detail(
        ...
        if detail is None:
            return None
        eval_report = await self._evals.get_eval_report(
    ...
    async def save_eval_report(self, report: EvalReport) -> None:
        await self._evals.save_eval_report(report)

    async def get_eval_report(
        self, interview_id: str, candidate_id: str | None = None
    ) -> EvalReport | None:
        return await self._evals.get_eval_report(interview_id, candidate_id=candidate_id)
